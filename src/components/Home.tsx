      ...
      >
        <div className="absolute inset-0 bg-black/60 bg-opacity-60 flex items-center justify-center text-center px-4">
          <div className="text-white max-w-3xl">
            ...
            <Link to="/about">
              <Button variant="secondary">Who We Are</Button>
            </Link>
          </div>
        </div>
      </section>
  ...
  return (
    <Card className="relative overflow-hidden rounded-xl shadow-md group transition hover:shadow-lg min-h-[320px] hover:cursor-pointer">
      ...
      <img
        src={image}
        alt={title}
        className="absolute inset-0 w-full h-full object-cover transition-transform duration-300 ease-in-out scale-100 group-hover:scale-105"
      />
      ...
      <div className="relative z-10 h-full flex flex-col justify-end md:justify-between p-4">
        ...
        <motion.div
          ...
          className="text-white text-xl font-semibold md:group-hover:cursor-pointer"
        >
          {title}
        </motion.div>

        {/* Description + CTA - hidden on desktop until hover, always visible on mobile */}
        <div className="mt-4 text-white md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300">
